 ...
 */
inline void opInLSFR(uint8_t mode = 0, uint32_t u = 0)
{
    uint32_t LLv = MulMod31((1u << 15), save_take_31bits(Reg[15])), RLv = MulMod31((1u << 17), save_take_31bits(Reg[13])),
             LRv = MulMod31((1u << 21), save_take_31bits(Reg[10])), RRv = MulMod31((1u << 20), save_take_31bits(Reg[4])),
             Lv = PlusMod31(LLv, RLv), Rv = PlusMod31(LRv, RRv),
             v = MulMod31((1u << 8 | 1u), save_take_31bits(Reg[0]));
    ...
    tmp = !mode ? PlusMod31(v, save_take_31bits(u)) : v;
...
inline void BitsReconstruction(uint32_t *x)
{
...
// 标准中定义为 L1 的 S 盒变换。
inline uint32_t
Linear_Transform1(uint32_t val) { return val ^ cyclic_Lshift(val, 2) ^ cyclic_Lshift(val, 10) ^ cyclic_Lshift(val, 18) ^ cyclic_Lshift(val, 24); }

// 标准中定义为 L2 的 S 盒变换。
inline uint32_t
Linear_Transform2(uint32_t val) { return val ^ cyclic_Lshift(val, 8) ^ cyclic_Lshift(val, 14) ^ cyclic_Lshift(val, 22) ^ cyclic_Lshift(val, 30); }

inline uint32_t NonLinearF(uint32_t *x)
{
